...
<section class="section">
  <div class="container article-grid">
    <div class="prose">
      ...
      <p><b>Честно о производстве:</b> наш цех находится в Иркутске, отдельной мастерской __INCITY__ у нас нет. Но __CITY__ — __DIST__ от Иркутска и входит в зону нашей работы, поэтому для вас доступны выезд на замер с образцами, доставка готового изделия и монтаж под ключ.</p>
      ...
      <p>Оставьте заявку → замерщик приедет __INCITY__ с образцами камня → изготовим изделие в Иркутске за 5–10 дней → привезём и установим __INCITY__. Стоимость выезда на замер и доставки уточним при заявке.</p>
...
foreach ($g in $geo) {
  ...
  $desc  = "Столешницы и изделия из искусственного камня $($g.inCity): кухни, ванные, мойки, подоконники. Производство в Иркутске, доставка и монтаж $($g.inCity). Замер и расчёт по заявке, гарантия 3 года."
  $faqPairs = @(
    ...
    @("Выезжаете на замер $($g.inCity)?", "Да, выезжаем на замер $($g.inCity) с образцами камня. Стоимость выезда уточним при заявке — она зависит от удалённости и нередко учитывается в стоимости заказа."),
